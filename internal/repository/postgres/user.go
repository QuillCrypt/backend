...
func (r *userRepository) Create(ctx context.Context, user *domain.User) error {
	query := `INSERT INTO users (id, username, email, avatar_url)
              VALUES ($1, $2, $3, $4) RETURNING created_at, updated_at`
	return r.db.QueryRow(ctx, query, user.ID, user.Username, user.Email, user.AvatarURL).
...
func (r *userRepository) GetByEmail(ctx context.Context, email string) (*domain.User, error) {
	user := &domain.User{}
	query := `SELECT id, username, email, avatar_url, created_at, updated_at FROM users WHERE email = $1`
